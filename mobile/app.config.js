...
// The official react-native-health-connect Expo plugin doesn't do this — it only adds
// an intent-filter. Without this call, requestPermission lateinit var is never initialized.
...
// Config plugins expect the FLAT format (no { expo: {} } wrapper)
const config = {
  name: 'MedBuddie',
  slug: 'medbuddie',
  version: '1.0.0',
...
// Apply the MainActivity plugin on Android — mods are preserved in the returned config
export default !IS_IOS ? withHealthConnectMainActivity(config) : config;
